...
// =============================================================================
// Histogram Matching in xyY Space with RGB Output
//
// This shader applies histogram matching to the Y (luminance) channel of xyY
// while preserving xy chromaticity, then converts xyY => RGB.
//
// Process:
...
// 6. Replace Y directly with targetLinearY (preserves color saturation)
// 7. Preserve xy chromaticity
// 8. Convert xyY => XYZ => ProPhoto RGB
// 9. Output RGB
// =============================================================================
...
void main() {
    ...
    // CRITICAL: The RAW CDF was built from original linear values that were gamma-encoded
    // WITHOUT normalization. Gamma encoding naturally compresses HDR values into [0,1],
    // so we must NOT normalize before gamma encoding - that would break the mapping!
    // Ensure Y is non-negative for gamma encoding
    float safeY = max(Y, 0.0);
    float gammaY = gammaEncode(safeY);
    
    // Evaluate histogram matching curve (clamps input internally to [0, 1])
    // The curve maps gamma-space values from RAW to gamma-space values from JPEG
    ...
    float newY = mix(Y, targetLinearY, histMatchStrength);
    // Only clamp to prevent negative values (not to limit HDR)
    newY = max(newY, 0.0);
    
    // Preserve xy chromaticity, update Y (direct replacement preserves color saturation)
    vec3 newXyY = vec3(x, y, newY);
    
    // Minimal validation - only fix truly invalid chromaticity
    // Be less aggressive to preserve original color
    ...
    // Convert xyY => XYZ => ProPhoto RGB
    vec3 XYZ = xyYtoXYZ(newXyY);
    vec3 proPhoto = XYZtoProPhoto * XYZ;
    // Only clamp truly negative values (shouldn't happen with valid xyY)
    proPhoto = max(proPhoto, vec3(0.0));
